...
class CommandThread(QThread):
    ...
    def _get_activated_command(self, cmd_list, new_console=False):
        ...
        activate_script = os.path.join(self.base_path, self.selected_env, script_folder, "activate.bat")
        ...
        if new_console:
            full_cmd = f'start cmd.exe /k "cd /d "{self.base_path}" && "{activate_script}" && {command_str}"'
            return full_cmd
        else:
            return f'cmd.exe /c "cd /d "{self.base_path}" && "{activate_script}" && {command_str}"'
...
class JupyterLauncher(QWidget):
    ...
    def _create_manage_venv_section(self):
        ...
        self.launch_tool_combo = QComboBox()
        self.launch_tool_combo.addItems(["Jupyter Notebook", "JupyterLab", "VS Code"])
        self.launch_tool_combo.setToolTip("Select a tool to launch within the environment.")
        self.launch_btn = QPushButton("Launch")
        self.launch_btn.setObjectName("launchBtn")
        self.launch_btn.setToolTip("Launch the selected tool.")
        launch_layout.addWidget(self.launch_tool_combo)
    ...
    def _apply_stylesheet(self):
        ...
        self.setStyleSheet(f"""
            ...
            #controlBtn, #recentBtn, #deleteBtn {{ background: transparent; border: none; font-size: 12pt; }}
            ...
            QComboBox QAbstractItemView::item {{ padding: 10px 6px; }}  /* <-- THIS LINE IS CHANGED */
    ...
    def _open_settings(self):
        ...
        if dialog.exec():
            ...
            self._load_app_settings()
            self._apply_stylesheet()
            self._update_status("Settings saved. Restart to apply theme fully.", "info")
    ...
    def _launch_tool(self):
        tool = self.launch_tool_combo.currentText()
        command = ""
        if tool == "Jupyter Notebook": command = "jupyter notebook"
        elif tool == "JupyterLab": command = "jupyter lab"
        elif tool == "VS Code": command = "code ."
        self._run_command("launch", tool=command)
    ...
    def discover_virtual_environments(self):
        ...
        all_widgets = [self.open_btn, self.new_venv_name_input, self.create_venv_btn, self.venv_dropdown,
                       self.delete_venv_btn, self.install_reqs_btn, self.freeze_btn, self.launch_tool_combo, self.launch_btn]
        env_dependent_widgets = [self.venv_dropdown, self.delete_venv_btn, self.install_reqs_btn, self.freeze_btn,
                                 self.launch_tool_combo, self.launch_btn]
