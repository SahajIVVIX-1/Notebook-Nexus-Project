...
# It's recommended to generate this file using: pyrcc6 resources.qrc -o resources_rc.py
# For a single-file solution, the content of resources_rc.py can be pasted here.
# As a placeholder, we'll check for icon files. Add 'icons/folder.png' and 'icons/file.png'.
    ...
    def run(self):
        try:
            ...
            if self.command_type == "jupyter":
                ...
                cmd = f'pip install jupyter'
            else: # "activate"
                # On Windows, 'activate' opens a new shell, which is what we want.
                # On Unix, we would need to handle it differently, but Popen suffices.
                cmd = None
            ...
            if sys.platform == "win32":
                ...
                # This is more complex on Unix as 'source' is a shell builtin.
                # A common approach is to launch a terminal with a command.
                # Example for gnome-terminal:
                # full_cmd = f'gnome-terminal --working-directory="{self.base_path}" -- bash -c "source {activate_script}; {cmd if cmd else "bash"};"'
                # This part needs to be adapted based on the target Unix environment.
                # For now, we'll emit a message.
                self.finished.emit(False, "Unix-like OS execution is not fully implemented.")
                return
...
class FileChangeHandler(FileSystemEventHandler):
    ...
    def on_any_event(self, event):
        if event.event_type in ['created', 'deleted', 'moved']:
            # Any of these events could mean a new venv was added/removed.
            self.callback()
...
class JupyterLauncher(QWidget):
    ...
    def _create_files_section(self):
        ...
        header_layout.addWidget(QLabel("Live Sync:"))
    ...
    def _apply_stylesheet(self):
        ...
        self.setStyleSheet(f"""
            ...
            QComboBox::drop-down {{ border: none; subcontrol-position: center right; subcontrol-origin: padding; }}
            QComboBox::down-arrow {{ image: url(icons/down_arrow.png); }} /* Add a 16x16 down arrow icon */
            ...
            #toggleBtn {{
                background-color: #30363d; border-radius: 11px;
                width: 40px; height: 22px; border: none;
            }}
            #toggleBtn::indicator {{
                background-color: #8b949e; border-radius: 9px;
                width: 18px; height: 18px; margin: 2px;
            }}
            #toggleBtn:checked {{ background-color: {self.COLOR_ACCENT}; }}
            #toggleBtn:checked::indicator {{ margin-left: 20px; }}
        """)
    ...
    def _on_path_changed(self):
        ...
        self._path_change_timer.start(500) # 500ms debounce
    ...
    def discover_virtual_environments(self):
        ...
        if is_path_valid:
            try:
                ...
                is_path_valid = False # Handle permission errors

        # Enable/disable path-dependent buttons
    ...
    def _update_status(self, message, msg_type):
        ...
        text_color = "white" if msg_type != "info" else self.COLOR_TEXT
        ...
        """)
        # Reset style after a delay for non-info messages
    ...
    def _set_progress_bar_active(self, is_active):
        ...
        if is_active:
            ...
            self.progress_bar.setRange(0, 100) # Stop indeterminate animation
            self.progress_bar.setVisible(False)

    def _set_default_path(self):
        """Sets an initial project path."""
        # Try a common development path first, otherwise use home.
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            # Only drag if the title bar is clicked
