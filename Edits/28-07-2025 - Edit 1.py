...
class CommandThread(QThread):
    ...
    def __init__(self, base_path, selected_env, command_type="jupyter", **kwargs):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            # Define the command to be executed based on the 'command_type'.
            if self.command_type == "jupyter":
                ...
                # Construct the command to install from a requirements file.
            ...
            else: # Corresponds to 'activate'
                cmd = None

            # Platform-specific execution logic.
            if sys.platform == "win32":
                # On Windows, activation scripts for 'cmd' have a .bat extension.
                activate_script += ".bat"
                # Before proceeding, check if the activation script actually exists.
                if not os.path.exists(activate_script):
                    raise FileNotFoundError(f"Activation script not found at: {activate_script}")

                # Construct the full command string for the Windows command prompt.
                # 1. `cd /d "{self.base_path}"`: Change directory to the project folder.
                # 2. `&& "{activate_script}"`: Activate the virtual environment.
                # 3. `&& {cmd}`: If a command is specified, run it after activation.
                ...
                # If the OS is not Windows, emit a 'finished' signal indicating failure.
                self.finished.emit(False, "Unix-like OS execution is not yet implemented.")
                return

            # A short delay to allow the new terminal to initialize.
            time.sleep(2)
            ...
            if self.command_type == "jupyter":
                ...
                self.jupyter_started.emit() # Signal that the process has started.
            elif self.command_type == "install_jupyter":
            ...
            # If the activation script wasn't found, emit a specific error.
            self.finished.emit(False, f"Error: A required file was not found. {str(e)}")
        except Exception as e:
            # Catch any other unexpected errors during thread execution and report them.
...
class JupyterLauncher(QWidget):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.resize(550, 650) # Increased height slightly for the new button
    ...
    # Creates the UI elements for managing the virtual environment.
    def _create_venv_section(self):
        ...
        # Layout for environment action buttons
        actions_layout = QVBoxLayout()
        actions_layout.setSpacing(8)

        # --- NEW: Button for installing requirements ---
        ...
        self.launch_btn.setObjectName("launchBtn") # Special ID for accent color styling.
    ...
    def _create_files_section(self):
        ...
        # Enable a custom context menu (right-click menu) for the file list.
    ...
    def _create_footer_section(self):
        ...
        # QSizeGrip provides a standard resize handle in the corner of the window.
    ...
    # Connects UI element signals (like button clicks) to their corresponding handler methods (slots).
    def _connect_signals(self):
        ...
        # --- NEW: Connect the new button's signal ---
    ...
    # Using a single stylesheet makes it easy to manage the application's look and feel.
    def _apply_stylesheet(self):
        # f-strings are used to inject the color and font constants into the CSS string.
    ...
    def _run_command(self, command_type, **kwargs):
        try:
            ...
            if not os.path.isdir(base_path) or not selected_env or "found" in selected_env or "Invalid" in selected_env:
            ...
            # Catch any errors that occur while trying to start the thread.
    ...
    # --- NEW: Method to handle installing libraries from a file ---
    @pyqtSlot()
    def _install_requirements(self):
        try:
            ...
            # Open a file dialog to select the requirements file.
            # It defaults to the current project path.
            ...
            # If the user selected a file, run the command.
            if file_path:
    ...
    def discover_virtual_environments(self):
        try:
            ...
            # --- Combined button list for enabling/disabling ---
            self.action_buttons = [
                self.open_btn, self.venv_dropdown, self.activate_btn, 
                self.install_btn, self.launch_btn, self.install_reqs_btn
            ]
            ...
            # Enable buttons now that the path is confirmed to be valid.
            for btn in self.action_buttons:
                btn.setEnabled(True)

            # A directory is considered a virtual environment if it contains the 'activate' script.
            script_folder = "Scripts" if sys.platform == "win32" else "bin"
            ...
            if not venvs:
                ...
                # If no env, disable all env-related actions
                for btn in [self.activate_btn, self.launch_btn, self.install_reqs_btn, self.install_btn]:
                    btn.setEnabled(False)
                ...
                # Enable all env-related actions
                for btn in [self.venv_dropdown, self.activate_btn, self.launch_btn, self.install_reqs_btn, self.install_btn]:
                    btn.setEnabled(True)
    ...
    def _update_file_observer(self):
        try:
            # If an observer is already running, stop it before starting a new one.
            if self.observer:
                self.observer.stop()
                self.observer.join()
                self.observer = None
            ...
            if os.path.isdir(path):
                ...
                # Schedule the FileChangeHandler to call discover_virtual_environments on any change.
    ...
    def _update_status(self, message, msg_type):
        try:
            ...
            # For success/error messages, reset the style back to normal after a few seconds.
