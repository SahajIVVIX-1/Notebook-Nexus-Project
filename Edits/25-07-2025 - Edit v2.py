...
from PyQt6.QtWidgets import (
    ...
    QListWidget, QMenu, QScrollArea, QCheckBox
)
...
class FileChangeHandler(FileSystemEventHandler):
    ...
    def on_any_event(self, event):
        if not event.is_directory and event.event_type in ['created', 'deleted', 'modified', 'moved']:
            self.callback()
        if event.is_directory and event.event_type in ['created', 'deleted', 'moved']:
            self.callback()
...
class JupyterLauncher(QWidget):
    ...
    def setup_ui(self):
        ...
        self.reload_btn.clicked.connect(self.discover_virtual_environments)
        ...
        self.monitor_checkbox = QCheckBox("Enable Real-Time File Monitoring")
        self.monitor_checkbox.setChecked(True)
        self.monitor_checkbox.stateChanged.connect(self.toggle_file_monitor)
        self.main_layout.addWidget(self.monitor_checkbox)
    ...
    def start_file_monitor(self):
        ...
        base_path = self.path_input.text().strip()
        if not os.path.isdir(base_path) or not self.monitor_checkbox.isChecked():
            return

        self.observer = Observer()
        event_handler = FileChangeHandler(self.discover_virtual_environments)
        self.observer.schedule(event_handler, base_path, recursive=False)
        self.observer.start()

    def toggle_file_monitor(self):
        if self.monitor_checkbox.isChecked():
            self.start_file_monitor()
            self.status_label.setText("Real-time file monitoring enabled")
        else:
            if hasattr(self, 'observer') and self.observer:
                self.observer.stop()
                self.observer.join()
                self.observer = None
            self.status_label.setText("Real-time file monitoring disabled")
